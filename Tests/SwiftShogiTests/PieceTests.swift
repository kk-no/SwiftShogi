...
final class PieceTests: XCTestCase {
    func testIsPromoted() {
        ...
        for item in kinds {
            let piece = Piece(kind: item.kind, color: .black)
            XCTAssertEqual(piece.isPromoted, item.expected)
        }
    }
    ...
    func testCanPromote() {
        ...
        for item in kinds {
            let piece = Piece(kind: item.kind, color: .black)
            XCTAssertEqual(piece.canPromote, item.expected)
        }
    }
    ...
    func testPromote() {
        ...
        for item in kinds {
            var piece = Piece(kind: item.kind, color: .black)
            piece.promote()

            let expected = Piece(kind: item.expectedKind, color: .black)
    ...
    func testAttacks() {
        ...
        for blackPiece in blackPieces {
            XCTAssertEqual(blackPiece.piece.attacks, Set(blackPiece.expectedArray))
        }
        ...
        for whitePiece in whitePieces {
            XCTAssertEqual(whitePiece.piece.attacks, Set(whitePiece.expectedArray))
        }
    }
    ...
    func testInitializerWithCharacter() {
        ...
        for item in characters {
            XCTAssertEqual(Piece(character: item.character, isPromoted: item.isPromoted), item.expected)
        }
    }
}
