...
final class SFENComponentsTests: XCTestCase {
    func testBoard() {
        ...
        for item in strings {
            let fields = SFEN.Fields(boardString: item.string)!
            XCTAssertEqual(fields.board, item.expected)
        }
    }
    ...
    func testColor() {
        ...
        for item in strings {
            let fields = SFEN.Fields(colorString: item.string)!
            XCTAssertEqual(fields.color, item.expected)
        }
    }
    ...
    func testCapturedPieces() {
        ...
        for item in strings {
            let fields = SFEN.Fields(capturedPiecesString: item.string)!
            XCTAssertEqual(fields.capturedPieces, item.expected)
        }
    }
}
